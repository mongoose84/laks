...
- For custom types (including strongly-typed IDs), implement `TryParse` or a `TypeConverter` so they bind from route and query values. Without one, the binder treats them as complex types and binding silently fails.
...
- Don't commit secrets to `appsettings.json`. Use `appsettings.{Environment}.json` for environment overrides, User Secrets (`dotnet user-secrets`) locally, and environment variables in production. Bind via `IOptions<T>`.

## Dependency Injection in PageModels

- Watch for the scoped-in-singleton captive dependency trap. If a singleton holds a reference to a scoped service (like an EF `DbContext`), that instance leaks across requests.
...
- In this project, data access goes through repository interfaces (`ICatchRepository`, `ISeasonRepository`) — never write raw SQL in PageModels or services. All queries must use parameterized queries (OWASP requirement).
...
- Unit-test `PageModel` classes directly. Instantiate them with mocked dependencies and assert on the returned `IActionResult`: `PageResult` for re-renders, `RedirectToPageResult` for successful PRG, `NotFoundResult` for 404 paths.
...
- Use `TimeProvider` (injected, optional parameter defaulting to `TimeProvider.System`) for any time-sensitive logic so tests can freeze the clock deterministically.
